...
public class MigrationInfoServiceImpl implements MigrationInfoService {
	...
	/**
	 * xuhaobo 2017-12-08 Refreshes the info about all known migrations from both
	 * the classpath and the DB.
	 */
	public void refresh() {
		...
		Map<String, MigrationInfoContext> contextMap = new HashMap<String, MigrationInfoContext>();
		...
		for (ResolvedMigration resolvedMigration : resolvedMigrations) {
			...
			if (version != null) {
				MigrationInfoContext verContext = contextMap.get(resolvedMigration.getModule());
				if (verContext == null) {
					verContext = new MigrationInfoContext();
					verContext.outOfOrder = outOfOrder;
					verContext.pending = pending;
					verContext.missing = missing;
					verContext.future = future;
					verContext.target = target;
					verContext.lastResolved = version;
					contextMap.put(resolvedMigration.getModule(), verContext);
				} else if (version.compareTo(verContext.lastResolved) > 0) {
					verContext.lastResolved = version;
				}
		...
		for (AppliedMigration appliedMigration : appliedMigrations) {
			...
			/*
			 * if (appliedMigration.getType() == MigrationType.SCHEMA) { context.schema =
			 * version; } if (appliedMigration.getType() == MigrationType.BASELINE) {
		...
		for (Pair<AppliedMigration, AppliedMigrationAttributes> av : appliedVersioned) {
			...
			MigrationInfoContext verContext = contextMap.get(av.getLeft().getModule());

			if (version != null) {
				if (verContext != null) {
					if (version.compareTo(verContext.lastResolved) > 0) {
						verContext.lastApplied = version;
						verContext.target = version;
					} else {
					...
					verContext.target = target;
					verContext.lastResolved = version;
					contextMap.put(av.getLeft().getModule(), verContext);
				}
			}
		}

		/*
		 * if (MigrationVersion.CURRENT == target) { context.target =
		 * context.lastApplied; }
		 */
		...
		for (Pair<AppliedMigration, AppliedMigrationAttributes> av : appliedVersioned) {
			ResolvedMigration resolvedMigration = resolvedVersioned
					.get(Pair.of(av.getLeft().getVersion(), av.getLeft().getType().isUndo()));
			if (resolvedMigration != null

					&& av.getLeft().isSuccess()) {
				pendingResolvedVersioned.remove(resolvedMigration);
			}
			migrationInfos1.add(new MigrationInfoImpl(resolvedMigration, av.getLeft(),
					contextMap.get(av.getLeft().getModule()), av.getRight().outOfOrder

			));
		}

		for (ResolvedMigration prv : pendingResolvedVersioned) {
			migrationInfos1.add(new MigrationInfoImpl(prv, null, contextMap.get(prv.getModule()), false

			));
		}

		for (AppliedMigration appliedRepeatableMigration : appliedRepeatable) {
			MigrationInfoContext context = contextMap.get(appliedRepeatableMigration.getModule());
			if (!context.latestRepeatableRuns.containsKey(appliedRepeatableMigration.getDescription())
					|| (appliedRepeatableMigration.getInstalledRank() > context.latestRepeatableRuns
							.get(appliedRepeatableMigration.getDescription()))) {
				context.latestRepeatableRuns.put(appliedRepeatableMigration.getDescription(),
						appliedRepeatableMigration.getInstalledRank());
			}
		}
		...
		for (AppliedMigration appliedRepeatableMigration : appliedRepeatable) {
			...
			if (resolvedMigration != null && appliedRepeatableMigration.getInstalledRank() == latestRank && ObjectUtils
					.nullSafeEquals(appliedRepeatableMigration.getChecksum(), resolvedMigration.getChecksum())) {
		...
		for (ResolvedMigration prr : pendingResolvedRepeatable) {
			MigrationInfoContext context = contextMap.get(prr.getModule());
	...
	public MigrationInfo current() {
		...
		// If no versioned migration has been applied so far, fall back to the latest
		// repeatable one
	...
	/**
	 * Retrieves the full set of infos about the migrations resolved on the
	 * classpath.
	 *
	...
	/**
	 * Retrieves the full set of infos about out of order migrations applied to the
	 * DB.
	 *
