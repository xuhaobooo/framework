...
public abstract class JavaMigrationResolver<M, E extends MigrationExecutor> implements MigrationResolver {
    ...
    public ResolvedMigrationImpl extractMigrationInfo(M migration) {
        ...
        String[] moduleArr = description.split("\\$\\$");
        if(moduleArr.length >1) {
        	resolvedMigration.setModule(moduleArr[0]);
        	resolvedMigration.setDescription(moduleArr[1]);
        }else {
        	resolvedMigration.setModule("public");
        	resolvedMigration.setDescription(moduleArr[0]);
        }
